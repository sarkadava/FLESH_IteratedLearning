...
datasets      <- paste0(parentfolder, '/R_stats/datasets/') # This line is different on mac and others
models        <- paste0(parentfolder, '/R_stats/models/')
plots         <- paste0(parentfolder, '/R_stats/plots/')
simfolder     <- paste0(parentfolder, '/ConceptSimilarity/')
...
# Adding communicative success

```{r com success}

similarity <- read_csv(paste0(simfolder, "conceptnet_cosine_similarity.csv"))
# in similarity, get rid of chain, generation and language
similarity <- similarity %>%
  select(-chain, -generation, -language) 

# in data, delete column chainID (bug)
data <- data %>% select(-chainID)

# create it again, by combining chain_number and generation_number (chXgY)
data <- data %>%
  mutate(chainID = paste0("ch", chain_number, "g", generation_number))

# rename gesture to concept
data <- data %>%
  rename(concept = gesture)

# merge data and similarity on chainID
data <- data %>%
  left_join(similarity, by = c("chainID", "concept"))
...
# show me relationship between diff_in_gesture_space and cosine_similarity
ggplot(data, aes(x = cosine_similarity, y = diff_in_gesture_space)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  geom_smooth(method = "loess", color = "red", linetype = "dashed") +
  labs(x = "Cosine Similarity", 
       y = "Difference in Gesture Space", 
       title = "Relationship between Difference in Gesture Space and Cosine Similarity") +
  theme_minimal()
```
Now for entropy
...
ggplot(data, aes(x = cosine_similarity, y = diff_in_entropy)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  labs(x = "Cosine Similarity", 
       y = "Difference in Entropy", 
       title = "Relationship between Difference in Gesture Space and Cosine Similarity") +
  theme_minimal()
...
ggplot(data, aes(x = cosine_similarity, y = diff_in_variability)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  # add loess dashed line
  geom_smooth(method = "loess", color = "red", linetype = "dashed") +
  labs(x = "Cosine Similarity", 
       y = "Difference in Temp. Variability", 
       title = "Relationship between Difference in Gesture Space and Cosine Similarity") +
  theme_minimal()
```
And total distance
...
ggplot(data, aes(x = cosine_similarity, y = total_distance)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  geom_smooth(method = "loess", color = "red", linetype = "dashed") +
  labs(x = "Cosine Similarity", 
...
What about cosine similarity as a function of generation

```{r}
# show me relationship between diff_in_gesture_space and cosine_similarity
ggplot(data, aes(x = generation_number, y = cosine_similarity)) +
  geom_point() +
  geom_smooth(method = "lm", color = "blue") +
  #geom_smooth(method = "loess", color = "red", linetype = "dashed") +
...
# 2) Zkus vytvořit jednoduchý testovací soubor v aktuálním working dir
test_path <- file.path(getwd(), "R_write_test.txt")
file.create(test_path)         # TRUE = vytvořeno, FALSE = selhalo
file.exists(test_path)         # TRUE/FALSE
# pokud byl vytvořen, smaž
if (file.exists(test_path)) file.remove(test_path)

# 3) Pokud máš konkrétní cestu, zkontroluj adresář a práva:
path <- "C:/cesta/k/souboru/out.csv"   # <-- nahraď svou cestou
dir.exists(dirname(path))
normalizePath(dirname(path), winslash = "/", mustWork = FALSE)
# check write permission (0 = writable)
file.access(dirname(path), 2)

# 4) Zkontroluj temp adresář (někdy balíky zapisují do temp)
tempdir()
file.access(tempdir(), 2)
...
```{r}
m5.gs <- brm(
  total_gesture_space2D ~ generation_number_c + Age + Extroversion + Gender + (1 | participantID) + (1 | concept) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 4000, warmup = 2000, cores = 4,
...
```{r}
ggplot(data, aes(x = Extroversion, y = total_distance, color = factor(diff_in_gender))) +
  geom_point(alpha = 0.5) +  # Scatter plot with transparency
...
# histogram of total_distance
hist(data$total_distance, n=50)
```


```{r}
m1.td <- brm(
  total_distance ~ generation_number_c + Age + Extroversion + Gender + cosine_similarity + (1 | participantID) + (1 | concept) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 4000, warmup = 2000, cores = 4,
  save_pars = save_pars(all = TRUE),
...
# Add criterions for later diagnostics
m1.td <- add_criterion(m1.td, criterion = c("loo", "waic"))

# Calculate also variance explained (R^2)
m1.td_R2 <- bayes_R2(m1.td)

# Save both as objects
saveRDS(m1.td, file.path(models, "m1.td.rds"))
saveRDS(m1.td_R2, file.path(models, "m1.td_R2.rds"))

beep(5)

m1.td <- readRDS(file.path(models, "m1.td.rds"))
m1.td_R2 <- readRDS(file.path(models, "m1.td_R2.rds"))
...
plot(conditional_effects(m1.td), points = TRUE)
# also here about what effect you see or not see

pp_check(m1.td, type = "dens_overlay")
# how well does the model predict future data? well. 

pp_check(m1.td, type = "error_scatter_avg")
# how are the values correlated with residual error? not really correlated too much
...
m2.td_s1 <- 
  brm(
    ...
    family = gaussian,
    total_distance ~ 1 +
      s(generation_number_c, bs = "bs", k = 19) +  # Smooth for Previous Answer similarity
      + Age + Extroversion + Gender + cosine_similarity + (1 | participantID) + (1 | concept) + (1 | chainID),  # Random effects
    iter = 4000, 
    warmup = 2000, 
    chains = 4, 
    cores = 4,
...
# Add criterions for later diagnostics
m2.td_s1 <- add_criterion(m2.td_s1, criterion = c("loo", "waic"))

# Calculate also variance explained (R^2)
m2.td_s1_R2 <- bayes_R2(m2.td_s1)

# Save both as objects
saveRDS(m2.td_s1, file.path(models, "m2.td_s1.rds"))
saveRDS(m2.td_s1_R2, file.path(models, "m2.td_s1_R2.rds"))
beep(5)
...
m2.td_s1 <- readRDS(file.path(models, "m2.td_s1.rds"))
m2.td_s1_R2 <- readRDS(file.path(models, "m2.td_s1_R2.rds"))
...
m2.td_s2 <- 
  brm(
    ...
    total_distance ~ 1 +
      s(generation_number_c, bs = "bs", k = 19) +  # Smooth for Previous Answer similarity
      + Age + Extroversion + Gender + cosine_similarity + (1 + generation_number_c || participantID) + (1 + generation_number_c || concept) + (1 + generation_number_c || chainID),  # Random effects
    iter = 4000, 
    warmup = 2000, 
    chains = 4, 
    cores = 4,
...
# Add criterions for later diagnostics
m2.td_s2 <- add_criterion(m2.td_s2, criterion = c("loo", "waic"))

# Calculate also variance explained (R^2)
m2.td_s2_R2 <- bayes_R2(m2.td_s2)

# Save both as objects
saveRDS(m2.td_s2, file.path(models, "m2.td_s2.rds"))
saveRDS(m2.td_s2_R2, file.path(models, "m2.td_s2_R2.rds"))
beep(5)
...
m2.td_s2 <- readRDS(file.path(models, "m2.td_s2.rds"))
m2.td_s2_R2 <- readRDS(file.path(models, "m2.td_s2_R2.rds"))
...
plot(conditional_effects(m2.td_s2), points = TRUE)


pp_check(m2.td_s2, type = "dens_overlay")
# looks ok but the ppd undershoots the low values and overshoot the high values, similar to the linear regression

pp_check(m2.td_s2, type = "error_scatter_avg")
# still not what we would like
...
Some diagnostics before improving model
```{r}
# rozložení odpovědi
hist(data$total_distance, breaks=50)
summary(data$total_distance)

# počet úrovní a velikosti skupin
table(data$participantID) |> summary()
table(data$concept)       |> summary()
table(data$chainID)       |> summary()

# korelace / multikolinearita mezi prediktory (jen numerické)
library(car)
vif(lm(total_distance ~ Age + Extroversion + cosine_similarity + generation_number_c, data=data))
cor(data[,c("Age","Extroversion","cosine_similarity","generation_number_c")], use="pairwise.complete.obs")



```

So collinearity is not the problem
...
num_vars <- c("Age","Extroversion","cosine_similarity")
data[paste0(num_vars,"_s")] <- scale(data[num_vars])

# log of the outcome
data$y_log <- log(data$total_distance + 1)   # +1 pokud může být nula
...
priors <- c(
  ...
  set_prior("exponential(1)", class = "sd"),     # group-level sds
  set_prior("exponential(1)", class = "sds")     # smoothing terms (if použiješ s())
)
```
...
form1 <- bf(y_log ~ 1 + s(generation_number_c, bs="tp", k=8) + Age_s + Extroversion_s + Gender + cosine_similarity_s +
             (1 | participantID) + (1 | concept) + (1 | chainID))

form2 <- bf(y_log ~ 1 + s(generation_number_c, bs="tp", k=8) + Age_s + Extroversion_s + Gender + cosine_similarity_s +
             (1 + generation_number_c | participantID) + (1 | concept) + (1 | chainID))
...
m_try <- brm(
  formula = form2,
  data = data,
  family = gaussian(),            # model na log-scale
  prior = priors,
...
summary(m_try)
pp_check(m_try)
plot(m_try)            # traceplots, Rhat
neff_ratio <- neff_ratio(m_try)
rhat <- rhat(m_try)
bayes_R2(m_try)
loo(m_try)             # porovnávej modely
...
plot(conditional_effects(m_try), points = TRUE)
...
# extract posterior samples for fixed effects (b_*)
post_fixed <- as.data.frame(posterior_samples(m_try, pars = "^b_"))

# bayesplot mcmc_areas (automatic ordering)
params_mat <- as.matrix(post_fixed)
bayesplot::mcmc_areas(params_mat, prob = 0.95) +
...
# candidate 3: try monotonic effect instead of a flexible spline
m_mono <- brm(y_log ~ 1 + mo(generation_number_c) + Age_s + Extroversion_s + Gender + cosine_similarity_s +
              (1 + generation_number_c | participantID) + (1 | concept) + (1 | chainID),
              data = data, family = gaussian(), control = list(adapt_delta=0.99))
```
...
summary(m_mono)
pp_check(m_mono)
plot(m_mono)            # traceplots, Rhat
neff_ratio <- neff_ratio(m_mono)
rhat <- rhat(m_mono)
bayes_R2(m_mono)
loo(m_mono)             # porovnávej modely
...
loo::loo_compare(loo(m_try), loo(m_mono))
```

```{r}
# extract posterior samples for fixed effects (b_*)
post_fixed <- as.data.frame(posterior_samples(m_mono, pars = "^b_"))
...
loo::loo_compare(loo(m_try), loo(m1.td), loo(m2.td_s1), loo(m2.td_s2))
```
...
# candidate 3: try monotonic effect instead of a flexible spline
m_inter <- brm(y_log ~ 1 + s(generation_number_c, bs="tp", k=8) + Age_s +
              (1 + generation_number_c | participantID) + (1 | concept) + (1 | chainID),
              data = data, family = gaussian(), control = list(adapt_delta=0.99))
```
```{r}
...
# Doesn't work becaus
loo::loo_compare(loo(m_try), loo(m_inter))
```
...
agg <- data %>%
  group_by(generation_number) %>%
  summarise(n = n(),
            med = median(total_distance, na.rm=TRUE),
            mean = mean(total_distance, na.rm=TRUE),
            lo = quantile(total_distance, 0.025, na.rm=TRUE),
            hi = quantile(total_distance, 0.975, na.rm=TRUE))

ggplot(agg, aes(x = generation_number, y = med)) +
  geom_point(aes(size = n)) +
  geom_errorbar(aes(ymin = lo, ymax = hi), width = 0.2, alpha = 0.5) +
  geom_smooth(data = data, aes(x = generation_number, y = total_distance), method = "loess", se = TRUE) +
  labs(y = "total_distance", title = "Median total_distance by generation (and LOESS)")

```
```{r}

# numeric correlation (generation numeric)
cor.test(data$generation_number, data$cosine_similarity, use = "pairwise.complete.obs")
cor.test(data$generation_number, data$Age, use = "pairwise.complete.obs")
cor.test(data$generation_number, data$Extroversion, use = "pairwise.complete.obs")

# tabulate generation by gender
table(data$generation_number, data$Gender)   # any imbalance?

```
Compare with and without generation   

```{r}

# assume you prepared: data$y_log (log distance) and standardized predictors Age_s, Extroversion_s, cosine_similarity_s
# and priors as earlier 'priors'

# model without generation
m_no_gen <- brm(
  y_log ~ Age_s + Extroversion_s + Gender + cosine_similarity_s +
    (1 | participantID) + (1 | concept) + (1 | chainID),
  data = data, family = gaussian(),
  iter = 4000, warmup = 2000, chains = 4,
  control = list(adapt_delta = 0.99, max_treedepth = 15), seed = 4
)

# model with generation (linear standardized)
m_with_gen <- brm(
  y_log ~ generation_number_c + Age_s + Extroversion_s + Gender + cosine_similarity_s +
    (1 | participantID) + (1 | concept) + (1 | chainID),
  data = data, family = gaussian(),
  iter = 4000, warmup = 2000, chains = 4,
  control = list(adapt_delta = 0.99, max_treedepth = 15), seed = 4
...
# LOO comparison (use reloo = TRUE if many high Pareto k)
loo_no   <- loo(m_no_gen, reloo = TRUE)
loo_with <- loo(m_with_gen, reloo = TRUE)
loo::loo_compare(loo_no, loo_with)


```
Show summaries of the model without generation
...
# plot effects with zero crossing using mcc_areas
post_fixed <- as.data.frame(posterior_samples(m_no_gen, pars = "^b_"))
bayesplot::mcmc_areas(as.matrix(post_fixed), prob = 0.95) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Posterior distributions of fixed effects (without generation)")



```
Adding cosine similarity as a random slope (people are differently sensitive to it)

```{r}

# priors <- c(
#   set_prior("normal(0, 1)", class = "b"),          # main effects
#   set_prior("normal(0, 0.5)", class = "b", coef = "cosine_similarity_s:Extroversion_s"),
#   set_prior("normal(0, 0.5)", class = "b", coef = "cosine_similarity_s:Age_s"),
#   # if Gender is treatment-coded, you'll have separate coefs per level; keep default 0,0.5 via 'class="b"' is fine
#   set_prior("normal(0, 2)", class = "Intercept"),
#   set_prior("student_t(3, 0, 2.5)", class = "sigma"),
...
# cosine × Extroversion
m1 <- update(m0, formula. = . ~ . + cosine_similarity_s:Extroversion_s)

# cosine × Age
m2 <- update(m0, formula. = . ~ . + cosine_similarity_s:Age_s)

# cosine × Gender  (if adequate sample per level)
m3 <- update(m0, formula. = . ~ . + cosine_similarity_s:Gender)

# random slope alternative (often best)
m4 <- brm(
  y_log ~ Age_s + Extroversion_s + Gender + cosine_similarity_s +
    (1 + cosine_similarity_s | participantID) + (1 | concept) + (1 | chainID),
  data = data, family = gaussian(),
  iter = 4000, warmup = 2000, chains = 4,
  control = list(adapt_delta = 0.99, max_treedepth = 15), seed = 4
)

loo::loo_compare(loo(m0), loo(m1), loo(m2), loo(m3), loo(m4))


```

So the main model with fixed effects only still seems to be the best. But the m2, m1 and m4 models are not necessarily that much worse. Only model including interaction of gender and cosine similarity seems to worsen significantly the predictive power

Let's now still look at m4, the model with random slope
```{r}
...
# plot effects with zero crossing using mcc_areas
post_fixed <- as.data.frame(posterior_samples(m4, pars = "^b_"))
bayesplot::mcmc_areas(as.matrix(post_fixed), prob = 0.95) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Posterior distributions of fixed effects (without generation)")
