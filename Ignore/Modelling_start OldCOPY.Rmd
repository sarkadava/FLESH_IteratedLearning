...
posterior_samples <- as_draws_df(m4.tv)
...
```
```{r}
ggplot(data, aes(x = Age, y = total_distance_c, color = factor(Gender))) +
  geom_point(alpha = 0.5) +  # Scatter plot with transparency
  geom_smooth(aes(group = factor(Gender)), method = "lm", se = TRUE) +  # Linear trend line with confidence interval
  labs(x = "Age", 
       y = "Gesture Space", 
...
```{r}
ggplot(data, aes(x = Age, y = total_gesture_space2D, color = factor(Gender))) +
  geom_point(alpha = 0.5) +  # Scatter plot with transparency
  geom_smooth(aes(group = factor(Gender)), method = "lm", se = TRUE) +  # Linear trend line with confidence interval
  labs(x = "Age", 
       y = "Gesture Space", 
...
```{r}
m1.dist <- brm(
  total_distance_c ~ diff_in_age + diff_in_extrovet + (1 | participantID) + (1 | gesture) + (1 | chain_number), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 2000, warmup = 1000, cores = 4,
  control = list(adapt_delta = 0.999, max_treedepth = 12)
