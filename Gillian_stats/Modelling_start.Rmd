...
- Does generation have an effect on temporal variaibility used?
...
```{r}
 c <- min(data$total_body_entropy2D[data$total_body_entropy2D > 0]) / 2 # decide on a constant
data$entropy_shifted <- data$total_body_entropy2D + c  # Shift all values
c2 <-min(data$total_body_variability2D[data$total_body_variability2D > 0]) / 2
data$variability_shifted <- data$total_body_variability2D + c2
```
...
m3.gs_R2
# explains 67.32 of the variance!
...
m4.gs <- brm(
  total_gesture_space2D ~ generation_number_c + Age + Extroversion + (1 | participantID) + (1 | gesture) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 2000, warmup = 1000, cores = 4, 
...
```
# MODEL FIVE: CENTERED AGE AND EXTROVERSION (Doesn't change anything in the end, so you can ignore this one)
```{r}
data$Age_c <- data$Age - median(range(data$Age, na.rm = TRUE))
data$Extroversion_c <- data$Extroversion - median(range(data$Extroversion,na.rm = TRUE))
```
```{r}
m5.gs <- brm(
  total_gesture_space2D ~ generation_number_c + Age_c + Extroversion_c + Gender + (1 | participantID) + (1 | gesture) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 2000, warmup = 1000, cores = 4,
...
loo(m5.gs, moment_match = TRUE)
loo_result <- loo(m5.gs, reloo = TRUE)
...
m2.et <- brm(
  log_entropy_shifted ~ generation_number_c, 
  data = data,
  family = lognormal(),  # Here we change lognormal
  chains = 4, iter = 2000, warmup = 1000, cores = 4
)
...
m3.et <- brm(
  log_entropy_shifted ~ generation_number_c + (1 | participantID) + (1 | gesture) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 4000, warmup = 2000, cores = 4
)
...
m4.et <- brm(
  entropy_shifted ~ generation_number_c + Age + Extroversion + Gender + (1 | participantID) + (1 | gesture) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 2000, warmup = 1000, cores = 4
)
...
m4.tv <- brm(
  variability_shifted ~ generation_number_c + Age_c + Extroversion_c + (1 | participantID) + (1 | gesture) + (1 | chainID), 
  data = data,
  family = lognormal(),  # Normal distribution for continuous entropy
  chains = 4, iter = 2000, warmup = 1000, cores = 4
)
...
# Questions about age-gesture space etc. (see model four)


# Questions about distance (rate of evolution)
# not even sure what to ask or what to do here
